...
public class Lexer {
    ...
    public boolean findTokens(){
        Java_files.TokenType[] tokenTypes= Java_files.TokenType.tokenTypeList;
        if(this.pos>=code.length())
            return false;
        for (int i=0;i<tokenTypes.length;i++){
            Java_files.TokenType tokenType=tokenTypes[i];
            ...
            if(matcher.find(this.pos)&&matcher.start()==this.pos)
            {
                ...
                if (is_new) {
                    ...
                    if(token.type!= Java_files.TokenType.tokenTypeList[3]&&token.type!= Java_files.TokenType.tokenTypeList[2]&&token.type!= Java_files.TokenType.tokenTypeList[1])
                        tokenList.add(token);
                }
                return true;
            }
